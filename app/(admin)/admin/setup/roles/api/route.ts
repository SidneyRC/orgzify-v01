...
export async function GET(req: NextRequest) {
  ...
  const scopedIds = await getScopedCompanyIds(session, { processId, module: 'assign_roles' })
  ...
  let query = supabaseAdmin.from('user_roles').select('id, user_id, company_id, role_id, status, users!user_roles_user_id_fkey(email), companies(display_name), admin_roles(name)', { count: 'exact' })
  ...
  const result = (rows || []).map((r: any) => {
    ...
    return { id: r.id, person_name: nameMap[r.user_id] || u?.email || '—', email: u?.email || '—', company_id: r.company_id, company_name: c?.display_name || '—', role_id: r.role_id, role_name: role?.name || '—', status: r.status || 'active' }
  })
...
export async function POST(req: NextRequest) {
  ...
  if (activeCompanyId) {
    ...
    if (!rights.assign_roles?.can_create) {
  ...
  const toInsert = user_ids.filter((id: string) => !existingIds.has(id)).map((id: string) => ({ user_id: id, company_id, role_id, assigned_by: session.user_id, created_by: session.user_id, is_active: true, status: 'active' }))
...
export async function PATCH(req: NextRequest) {
  ...
  const { id, status, company_id, role_id } = await req.json()
  if (!id) return NextResponse.json({ error: 'Record is missing.' }, { status: 400 })

  const { data: target } = await supabaseAdmin.from('user_roles').select('company_id').eq('id', id).maybeSingle()
  const activeCompanyId = await getActiveCompanyId(req.cookies)
  if (isOwnRecord(activeCompanyId, target?.company_id)) {
    return NextResponse.json({ error: 'You cannot edit your own company\'s assignment' }, { status: 403 })
  }
  if (activeCompanyId) {
    const rights = await getFullCompanyRights(session.user_id, activeCompanyId)
    if (!rights.assign_roles?.can_edit) {
      return NextResponse.json({ error: 'You do not have permission to edit assignments' }, { status: 403 })
    }
  }

  const fields: any = { updated_at: new Date().toISOString() }
  if (status) { fields.status = status; fields.is_active = status === 'active' }
