// OREV1-044 — Admin Setup — Countries — /admin/setup/countries
"use client";
...
export default function CountriesPage() {
  ...
  const loadCountries = useCallback(async () => {
    ...
    else showToast("error", "Failed to load countries.");
    setLoading(false);
  }, []);
  ...
  const handleSync = async () => {
    ...
    const data = await res.json();
    if (res.ok) { showToast("success", `Sync complete — ${data.count} countries loaded.`); loadCountries(); }
...
<div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-5">
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-gray-400 text-xs mt-1">Click Sync Countries to load data.</p>
        </div>
      ) : (
