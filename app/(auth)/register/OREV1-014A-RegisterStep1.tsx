...
export default function RegisterStep1({ onNext }: Props) {
  ...
  return (
    <>
      <h1 className="text-2xl font-bold text-blue-900 mb-1">Create your account</h1>
      <p className="text-gray-500 text-sm mb-6">Step 1 of 3 — Basic information</p>

      {/* OAuth */}
      <div className="space-y-3 mb-5">
        <button onClick={() => handleOAuth("google")}
          className="w-full flex items-center justify-center gap-3 border-2 border-gray-200 rounded-xl py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
        ...
        <button onClick={() => handleOAuth("facebook")}
          className="w-full flex items-center justify-center gap-3 bg-[#1877F2] rounded-xl py-3 text-sm font-medium text-white hover:bg-[#1565D8] transition-colors">
      ...
      <div className="relative mb-5">
        ...
        <div className="relative flex justify-center text-xs uppercase">
          <span className="bg-white px-3 text-gray-400 font-medium tracking-wider">Or register with email</span>
        </div>
      </div>
        ...
        <div>
          ...
          <div className="flex gap-2">
            <select value={title} onChange={e => setTitle(e.target.value as Title)}
              className="border border-gray-200 rounded-xl px-3 py-3 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-900/20 focus:border-blue-900 bg-white">
              {(["Mr.", "Mrs.", "Ms.", "Dr.", "Prof."] as Title[]).map(t => <option key={t}>{t}</option>)}
            </select>
            <input type="text" value={fullName} onChange={e => setFullName(e.target.value)}
              placeholder="Your full name"
              className="flex-1 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-900/20 focus:border-blue-900 transition-colors" />
          </div>
        </div>
