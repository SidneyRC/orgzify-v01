// THIS FILE GOES IN: components/admin/OREV1-119B-ArtistsTable.tsx (NEW FILE)
'use client'
...
type Props = { rows: ArtistRow[]; loading: boolean; theme: any; STATUS_LABELS: Record<string, string>; STATUS_COLORS: Record<string, string>; onApprove: (r: ArtistRow) => void; onReject: (r: ArtistRow) => void; onEdit: (r: ArtistRow) => void; onToggleEnabled: (r: ArtistRow) => void; onDelete: (r: ArtistRow) => void }
...
function StatusActions({ r, onApprove, onReject, onEdit, onDelete }: { r: ArtistRow; onApprove: (r: ArtistRow) => void; onReject: (r: ArtistRow) => void; onEdit: (r: ArtistRow) => void; onDelete: (r: ArtistRow) => void }) {
  return (
    <div className="flex gap-3 items-center">
      ...
      {r.status !== 'active' && <button onClick={() => onDelete(r)} title="Delete" className="text-gray-400 hover:text-red-600 transition p-1 rounded-lg hover:bg-red-50"><IconDelete /></button>}
    </div>
  )
}

function EnableToggle({ r, onToggle, theme }: { r: ArtistRow; onToggle: (r: ArtistRow) => void; theme: any }) {
  if (r.status !== 'active') return <span className="text-gray-300 text-xs">—</span>
  return (
...
export default function OREV1119BArtistsTable({ rows, loading, theme, STATUS_LABELS, STATUS_COLORS, onApprove, onReject, onEdit, onToggleEnabled, onDelete }: Props) {
    ...
    <div className="hidden md:block bg-white rounded-2xl border border-gray-100 overflow-x-auto">
      <table className="w-full text-sm">
        <thead style={{ backgroundColor: theme?.table_header_bg || '#f9fafb' }}>
          <tr>{['Photo', 'Name', 'Speciality', 'Created', 'Status', 'Enabled', 'Actions'].map(h => <th key={h} className="text-left px-4 py-3 text-xs font-semibold" style={{ color: theme?.table_header_text || '#6b7280' }}>{h}</th>)}</tr>
        </thead>
        <tbody>
          {loading && <tr><td colSpan={7} className="text-center py-12 text-sm text-gray-400">Loading…</td></tr>}
          {!loading && rows.length === 0 && <tr><td colSpan={7} className="text-center py-12 text-sm text-gray-400">No artists found.</td></tr>}
          {!loading && rows.map(r => (
            <tr key={r.id} className="border-t border-gray-50 hover:bg-gray-50 transition">
              <td className="px-4 py-3">{r.photo_url ? <img src={r.photo_url} onClick={() => setZoomedUrl(r.photo_url)} className="w-14 h-14 rounded-full object-cover cursor-zoom-in hover:opacity-80 transition" /> : <span className="w-14 h-14 rounded-full bg-gray-100 inline-block" />}</td>
              <td className="px-4 py-3 font-medium" style={{ color: theme?.color_text_primary || '#111827' }}>{r.name}</td>
              <td className="px-4 py-3 text-gray-500">{r.speciality_name}</td>
              ...
              <td className="px-4 py-3"><StatusActions r={r} onApprove={onApprove} onReject={onReject} onEdit={onEdit} onDelete={onDelete} /></td>
            </tr>
          ))}
        </tbody>
      </table>
