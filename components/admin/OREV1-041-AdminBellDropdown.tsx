...
export default function AdminBellDropdown({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="absolute right-0 top-full mt-2 w-80 bg-white border border-gray-200 rounded-xl shadow-lg z-50 overflow-hidden">
            ...
            <div key={n.id} onClick={() => markRead(n.id)}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 mb-0.5">
                  {!n.read && <span className="w-1.5 h-1.5 rounded-full bg-blue-900 shrink-0" />}
                  <span className={`text-xs font-medium ${n.read ? 'text-gray-500' : 'text-gray-800'}`}>{n.title}</span>
                </div>
                <p className="text-xs text-gray-500 leading-snug truncate">{n.description}</p>
      ...
      <div className="px-4 py-2.5 border-t border-gray-100 text-center">
        <button className="text-xs font-medium text-blue-900 hover:underline">View all notifications →</button>
      </div>
    </div>
  );
}