// THIS FILE GOES IN: components/admin/OREV1-118-SponsorsToolbar.tsx (NEW FILE)
'use client'
type Props = {
...
export default function OREV1118SponsorsToolbar({ theme, statusFilter, setStatusFilter, searchInput, setSearchInput, page, setPage, limit, setLimit, total, totalPages, STATUS_LABELS, onSearch, onReset, onDownload, onAdd, onBack }: Props) {
  ...
  return (
    <>
      <div className="flex items-center justify-between gap-3 mb-5 flex-wrap">
        <h1 className="text-2xl font-semibold" style={{ color: theme?.color_text_primary || '#111827' }}>Sponsors</h1>
        <div className="flex gap-2">
          ...
          <button onClick={onAdd} style={primaryBtn} className="text-sm font-medium px-4 py-2 hover:opacity-90">+ Add Sponsor</button>
        </div>
      </div>
      <div className="mb-3">
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className={selectClass}>
          <option value="">All Statuses</option>
          {['pending', 'approved', 'rejected'].map(s => <option key={s} value={s}>{STATUS_LABELS[s]}</option>)}
        </select>
      </div>
      <div className="flex gap-2 mb-5 items-center">
        <input value={searchInput} onChange={e => setSearchInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') onSearch() }} placeholder="Search by sponsor name…" className="flex-1 border border-gray-200 rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-blue-400" />
