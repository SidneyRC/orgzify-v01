// THIS FILE GOES IN: components/admin/OREV1-119C-ArtistsMobileCards.tsx (NEW FILE)
'use client'
...
export default function OREV1119CArtistsMobileCards({ rows, loading, theme, STATUS_LABELS, STATUS_COLORS, onApprove, onReject, onEdit, onToggleEnabled, onDelete }: Props) {
  ...
  return (
    <div className="md:hidden flex flex-col gap-3">
      ...
      {!loading && rows.length === 0 && <p className="text-center py-12 text-sm text-gray-400">No artists found.</p>}
      {!loading && rows.map(r => (
        <div key={r.id} className="bg-white rounded-2xl border border-gray-100 p-4 flex flex-col gap-3 shadow-sm">
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-center gap-3 flex-1 min-w-0">
              {r.photo_url ? <img src={r.photo_url} onClick={() => setZoomedUrl(r.photo_url)} className="w-12 h-12 rounded-full object-cover cursor-zoom-in shrink-0" /> : <span className="w-12 h-12 rounded-full bg-gray-100 inline-block shrink-0" />}
              <div className="min-w-0">
                <p className="font-semibold text-sm truncate" style={{ color: theme?.color_text_primary || '#111827' }}>{r.name}</p>
                <p className="text-xs text-gray-400 truncate">{r.speciality_name}</p>
              </div>
            </div>
