// GOES IN: components/admin/OREV1-077-CategoriesPage.tsx
'use client'
import { useState, useEffect, useCallback } from 'react'
import { useRouter } from 'next/navigation'
import { useTheme } from '@/lib/ThemeContext'
import toast from 'react-hot-toast'
import OREV1077ACategoryFormModal from '@/components/admin/OREV1-077A-CategoryFormModal'
...
type CategoryRow = { id: string; name: string; level: number; status: string; parent: { name: string } | null }
...
export default function OREV1077CategoriesPage({ canCreate = true, canEdit = true, canDelete = true, canViewArchived = true, canRestore = true, canDownload = true, canHardDelete = false }: Props) {
  const router = useRouter()
  const { theme } = useTheme()
  const [rows, setRows] = useState<CategoryRow[]>([])
  const [l1Options, setL1Options] = useState<{ id: string; name: string }[]>([])
  ...
  const selectClass = "w-full border border-gray-200 rounded-xl px-3 py-2 text-sm bg-white"
  ...
  const fetchData = useCallback(async () => {
    ...
    const res = await fetch(`/admin/master/categories/api?${params}`)
    const json = await res.json()
    if (json.error) { toast.error('Failed to load categories'); setLoading(false); return }
    setRows(json.data || []); setTotal(json.total || 0); setLoading(false)
  }, [page, limit, search, levelFilter, statusFilter])

  const fetchL1Options = useCallback(async () => {
    const res = await fetch('/admin/master/categories/api?level=1&status=active&limit=200')
    const json = await res.json()
    setL1Options((json.data || []).map((c: any) => ({ id: c.id, name: c.name })))
  }, [])

  useEffect(() => { fetchData() }, [fetchData])
  useEffect(() => { fetchL1Options() }, [fetchL1Options])

  const handleSearch = () => { setSearch(searchInput); setPage(1) }
  const handleReset = () => { setSearchInput(''); setSearch(''); setLevelFilter(''); setStatusFilter(''); setPage(1) }

  const handleToggleActive = async (row: CategoryRow) => {
    const nextStatus = row.status === 'active' ? 'inactive' : 'active'
    setRows(rows => rows.map(x => x.id === row.id ? { ...x, status: nextStatus } : x))
    const res = await fetch('/admin/master/categories/api', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: row.id, status: nextStatus }) })
  ...
  const handleDelete = async (row: CategoryRow) => {
    if (!confirm('Move this category to Archived?')) return
    const res = await fetch(`/admin/master/categories/api?id=${row.id}`, { method: 'DELETE' })
    const json = await res.json()
    if (json.error) { toast.error(json.error); return }
    toast.success('Category archived.'); fetchData()
  }
  const handleRestore = async (row: CategoryRow) => {
    const res = await fetch('/admin/master/categories/api', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: row.id, action: 'restore' }) })
    const json = await res.json()
    if (json.error) { toast.error(json.error); return }
    toast.success('Restored — now Inactive.'); fetchData()
  }
  const handleHardDelete = async (row: CategoryRow) => {
    if (!confirm('Permanently delete this category? This CANNOT be undone.')) return
    const res = await fetch(`/admin/master/categories/api?id=${row.id}&hard=true`, { method: 'DELETE' })
    const json = await res.json()
    if (json.error) { toast.error(json.error); return }
    toast.success('Permanently deleted.'); fetchData()
  }
  const handleDownload = () => window.open(`/admin/master/categories/api?limit=1000&format=csv`, '_blank')
  const goBack = () => router.push('/admin/master')
  const openView = (r: CategoryRow) => { setEditing(r); setViewOnly(true); setModalOpen(true) }
  const openEdit = (r: CategoryRow) => { setEditing(r); setViewOnly(false); setModalOpen(true) }
  ...
  return (
    <div className="p-6 pr-8" style={{ backgroundColor: theme?.page_bg }}>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <h1 className="text-2xl font-semibold" style={{ color: theme?.color_text_primary || '#111827' }}>Categories</h1>
      ...
      <div className="hidden md:block bg-white rounded-2xl border border-gray-100 overflow-x-auto">
        <table className="w-full text-sm">
          <thead style={{ backgroundColor: theme?.table_header_bg || '#f9fafb' }}>
            <tr>{['Name', 'Level', 'Parent', 'Status', 'Actions'].map(h => <th key={h} className="text-left px-4 py-3 text-xs font-semibold" style={{ color: theme?.table_header_text || '#6b7280' }}>{h}</th>)}</tr>
          </thead>
          <tbody>
            {loading && <tr><td colSpan={5} className="text-center py-12 text-sm text-gray-400">Loading…</td></tr>}
            {!loading && rows.length === 0 && <tr><td colSpan={5} className="text-center py-12 text-sm text-gray-400">No categories found.</td></tr>}
            {!loading && rows.map(r => (
              <tr key={r.id} className="border-t border-gray-50">
                <td className="px-4 py-3 font-medium">{r.parent ? `${r.parent.name} > ${r.name}` : r.name}</td>
                <td className="px-4 py-3 text-gray-500">{r.level === 1 ? 'Category' : 'Sub Category'}</td>
                <td className="px-4 py-3 text-gray-400">{r.parent?.name || '—'}</td>
        ...
        {!loading && rows.map(r => (
          <div key={r.id} className="bg-white rounded-2xl border border-gray-100 p-4 flex flex-col gap-3">
            <div className="flex justify-between items-start">
              <p className="font-semibold text-sm">{r.parent ? `${r.parent.name} > ${r.name}` : r.name}</p>
              <StatusCell r={r} />
            </div>
            <div className="flex justify-between text-xs text-gray-500"><span>Level</span><span>{r.level === 1 ? 'Category' : 'Sub Category'}</span></div>
            <ActionIcons r={r} />
          </div>
        ))}
      </div>

      {modalOpen && <OREV1077ACategoryFormModal editing={editing} l1Options={l1Options} readOnly={viewOnly} onClose={() => setModalOpen(false)} onSaved={() => { setModalOpen(false); fetchData(); fetchL1Options() }} />}
    </div>
  )

  function StatusCell({ r }: { r: CategoryRow }) {
  ...
  function ActionIcons({ r }: { r: CategoryRow }) {
