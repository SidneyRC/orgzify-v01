...
export default function EntityDashboardCards({ processId, moduleAccess, theme, radius }: {
  processId: string; moduleAccess: ModuleAccess; theme: Theme | null; radius: string;
}) {
  ...
  const cards: { key: string; Icon: LucideIcon; title: string; desc: string; href?: string; show: boolean }[] = [
    ...
    { key: "events", Icon: CalendarDays, title: "Events", desc: "Events creation and setup", show: moduleAccess.events },
  ].filter(c => c.show);
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {cards.map(({ key, Icon, title, desc, href }) => {
        const body = (
          <div style={cardStyle} className="bg-white p-5 h-full transition-shadow hover:shadow-lg cursor-pointer">
        ...
        return href ? <a key={key} href={href}>{body}</a> : <div key={key}>{body}</div>;
      })}
    </div>
  );
}
